define(["api/SplunkVisualizationBase","api/SplunkVisualizationUtils","splunkjs/mvc"], function(__WEBPACK_EXTERNAL_MODULE_3__, __WEBPACK_EXTERNAL_MODULE_4__, __WEBPACK_EXTERNAL_MODULE_5__) { return /******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
...
/***/ (function(module, exports, __webpack_require__) {
	...
	class departures_board{
		...
		setConfig(config, namespace, mvc){
			// Get Config parameters:
				...
				if(this.theme!="light" && this.theme!="dark" && this.theme!="custom") {
					this.theme="light";
				}
				
				
				
				const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.test(this.customColor);
		...
		getHTML(){
			...
			var html = "<style>" + this.getCSSForCustomColors() + "</style><div class=\"dbv_container\" style=\"" + cssAlign + customCSS +  "\"><div class=\"dvb_sub_container\"><input class=\"header " + cssTheme + " " + this.size +"\" id=\"" + this.id + "\" /><div class=\"dbv_label\">" + this.label + "</div></div></div>";
			return html;
		}
	...
	class CSSColorSolver {
	  ...
	  css(filters) {
	    ...
	    return `filter: saturate(100%) invert(${fmt(0)}%) sepia(${fmt(1)}%) saturate(${fmt(2)}%) hue-rotate(${fmt(3, 3.6)}deg) brightness(${fmt(4)}%) contrast(${fmt(5)}%);`;
	  }
	}
