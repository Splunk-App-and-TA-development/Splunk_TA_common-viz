// https://github.com/syntagmatic/parallel-coordinates ( 2016-04-19 )
// changes:
...
d3.parcoords = function (config) {
            ...
            if (i < cols - 1) {
                ...
                if (__.bundleDimension !== null) {
                    var leftCentroid = __.clusterCentroids
                        .get(__.dimensions[__.bundleDimension].yscale(row[__.bundleDimension]))
                        .get(p[i]);
    ...
    function single_path(d, ctx) {
        d3.entries(__.dimensions).forEach(function (p, i) {
            ...
            if (i == 0) {
                ctx.moveTo(
                    position(p.key),
                    typeof d[p.key] == 'undefined' ? getNullPosition() : __.dimensions[p.key].yscale(d[p.key]),
                );
            } else {
                ctx.lineTo(
                    position(p.key),
    ...
    function rotateLabels() {
        ...
        pc.svg.selectAll('text.label').attr('transform', 'translate(0,-10) rotate(' + __.dimensionTitleRotation + ')');
