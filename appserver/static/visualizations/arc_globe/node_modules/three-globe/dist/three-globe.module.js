import { SphereGeometry, Color, LineBasicMaterial, LineSegments, Mesh, MeshPhongMaterial, TextureLoader, CylinderGeometry, CylinderBufferGeometry, FaceColors, Geometry, Matrix4, MeshBasicMaterial, MeshLambertMaterial, Object3D, AdditiveBlending, BufferGeometry, CubicBezierCurve3, Curve, Float32BufferAttribute, Line, QuadraticBezierCurve3, ShaderMaterial, TubeBufferGeometry, Vector3, DoubleSide, Group, NoColors, VertexColors, Font, CircleBufferGeometry, TextBufferGeometry, Vector2 } from 'three';
...
var THREE = window.THREE ? _objectSpread2({}, window.THREE, {
...
var PathsLayerKapsule = Kapsule({
  ...
  update: function update(state) {
    ...
    var sharedShaderMaterial = new THREE$7.ShaderMaterial(_objectSpread2({}, gradientShaders$1, {
      transparent: true,
...
var Globe = Kapsule({
  ...
  init: function init(threeObj, state, _ref10) {
    ...
    threeObj.add(state.scene = new THREE$9.Group()); // add globe layer group

    var globeG = new THREE$9.Group();
    state.scene.add(globeG);
    state.globeLayer(globeG); // add points layer group

    var pointsG = new THREE$9.Group();
    state.scene.add(pointsG);
    state.pointsLayer(pointsG); // add arcs layer group

    var arcsG = new THREE$9.Group();
    state.scene.add(arcsG);
    state.arcsLayer(arcsG); // add hexbin layer group

    var hexBinG = new THREE$9.Group();
    state.scene.add(hexBinG);
    state.hexBinLayer(hexBinG); // add polygons layer group

    var polygonsG = new THREE$9.Group();
    state.scene.add(polygonsG);
    state.polygonsLayer(polygonsG); // add hexed polygons layer group

    var hexedPolygonsG = new THREE$9.Group();
    state.scene.add(hexedPolygonsG);
    state.hexedPolygonsLayer(hexedPolygonsG); // add paths layer group

    var pathsG = new THREE$9.Group();
    state.scene.add(pathsG);
    state.pathsLayer(pathsG); // add labels layer group

    var labelsG = new THREE$9.Group();
    state.scene.add(labelsG);
    state.labelsLayer(labelsG); // add custom layer group

    var customG = new THREE$9.Group();
